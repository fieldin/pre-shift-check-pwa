...
const mockChecklistItems = [
  { item_id: 'T01', text: 'Engine oil level is acceptable', priority: 'HIGH' as const, sort_order: 1 },
  { item_id: 'T02', text: 'Coolant level is adequate', priority: 'HIGH' as const, sort_order: 2 },
  { item_id: 'T03', text: 'Tires are in good condition', priority: 'MED' as const, sort_order: 3 },
];
...
describe('Response Validation', () => {
  ...
  function validateResponse(response: CheckResponse): { valid: boolean; error?: string } {
    if (!response.item_id) {
  ...
  it('should accept YES answer without comment', () => {
    const response: CheckResponse = { item_id: 'T01', answer: 'YES' };
    const result = validateResponse(response);
    expect(result.valid).toBe(true);
  });

  it('should accept YES answer with optional comment', () => {
    const response: CheckResponse = { item_id: 'T01', answer: 'YES', comment: 'All good' };
    const result = validateResponse(response);
    expect(result.valid).toBe(true);
  });

  it('should accept NO answer with comment', () => {
    const response: CheckResponse = { item_id: 'T01', answer: 'NO', comment: 'Oil level is low' };
    const result = validateResponse(response);
    expect(result.valid).toBe(true);
  });

  it('should reject NO answer without comment', () => {
    const response: CheckResponse = { item_id: 'T01', answer: 'NO' };
    const result = validateResponse(response);
  ...
  it('should reject NO answer with empty comment', () => {
    const response: CheckResponse = { item_id: 'T01', answer: 'NO', comment: '   ' };
    const result = validateResponse(response);
    expect(result.valid).toBe(false);
  });

  it('should reject response without item_id', () => {
    const response = { answer: 'YES' } as CheckResponse;
    const result = validateResponse(response);
    expect(result.valid).toBe(false);
...
describe('PASS/FAIL Calculation', () => {
  ...
  function calculateResult(responses: CheckResponse[]): 'PASS' | 'FAIL' {
...
describe('Sync Upsert Flow', () => {
  ...
  let mockDb: Map<string, Event>;

  beforeEach(() => {
    mockDb = new Map();
  });

  function upsertEvent(event: Event): { action: 'created' | 'updated'; event: Event } {
  ...
  it('should create new event when not exists', () => {
    const event: Event = { event_id: 'EVT-001', asset_id: 'TRAC001', result: 'PASS' };
  ...
  it('should update existing event when exists', () => {
    // First create
    const event1: Event = { event_id: 'EVT-001', asset_id: 'TRAC001', result: 'PASS' };
    const result1 = upsertEvent(event1);
    const originalCreatedAt = result1.event.created_at;

    // Then update
    const event2: Event = { event_id: 'EVT-001', asset_id: 'TRAC001', result: 'FAIL' };
...
describe('Fault Creation from Failed Checks', () => {
  ...
  function createFaultsFromResponses(
    eventId: string,
    assetId: string,
    ...
    return noResponses.map((response, index) => {
      ...
      return {
        ...
        origin: 'PRE_SHIFT_CHECK' as const,
